...
const AuthProvider = ({ children }) => {
    ...
    useEffect(() => {
        const unSubscribe = onAuthStateChanged(auth, currentUser => {
            ...
            if(currentUser){
                axios.post('https://job-seeking-server-eta.vercel.app/jwt', loggedUser,{withCredentials: true})
                .then(res=>{
                    console.log('token response', res.data);
                })
            }
            else{
                axios.post('https://job-seeking-server-eta.vercel.app/logout',loggedUser, {withCredentials: true})
                .then(res=>{
